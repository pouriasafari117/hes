...
function bindLogin(){
  ...
  form.addEventListener('submit', async e => {
    ...
    // ۱) اگر حالت سرور فعال است، اول به API بزن
    try {
      if(typeof SRV !== 'undefined' && SRV.base){
    ...
    setTimeout(()=>{
      ...
      const passOk = user && (rawP==='1234' || toEn(user.nationalId||'')===rawP || toEn(user.nid||'')===rawP || rawP===user.username);
      if(!user || !passOk){
        btn.disabled = false; txt.textContent = 'ورود به سامانه';
...
/* ── کارت تنظیمات حذف شد ── */
function injectSrvSec(){ /* no-op - removed per Round 32 */ }
...
async function submitOnboarding(){
  ...
  // اول سعی کن به سرور بزنی، اگر نشد برو حالت دمو
  let serverOk = false;
  try {
    if (typeof SRV !== 'undefined' && SRV.base) {
...
function patchSettings(){
  ...
  if (origRenderSettings && !origRenderSettings._patched) {
    window.renderSettings = function(){
      ...
      // داده‌ها و ممیزی: فقط بازنشانی دمو
      const dataBox = document.querySelector('#secData .sec-b');
      if (dataBox) {
        const wipeBtn = dataBox.querySelector('#setWipe');
        if (wipeBtn) {
          const row = wipeBtn.closest('.setting-row');
          if (row) row.remove();
        }
      }
