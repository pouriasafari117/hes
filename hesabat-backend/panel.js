...
function bindLogin(){
  ...
  $('#lnkForgot').addEventListener('click', e => {
    ...
    openModal({ size:'sm', title:'بازیابی رمز عبور',
      body:'<div class="fields" style="grid-template-columns:1fr">' +
        '<div class="field"><label>نام کاربری یا شماره موبایل</label><input id="fgUser" placeholder="مثلاً admin یا 0912…"><span class="err-msg"></span>' +
        '<span class="help">در نسخه کامل، کد بازیابی پیامک می‌شود.</span></div></div>',
      ...
      onOpen(h){ h.el.querySelector('[data-x]').onclick = ()=>h.close();
        h.el.querySelector('#fgSend').onclick = ()=>{
          ...
          if(v.length < 3){ markErr($('#fgUser'),'نام کاربری یا موبایل را وارد کنید.'); return; }
          h.close(); toast('اگر این حساب وجود داشته باشد، لینک بازیابی ارسال شد. (دمو)','ok');
        }; } });
  });
  form.addEventListener('submit', e => {
    e.preventDefault();
    const u = fieldVal('#lgUser').trim(), p = $('#lgPass').value;
    const alertBox = $('#loginAlert');
    clearErr($('#lgUser')); clearErr($('#lgPass'));
    alertBox.innerHTML = '';
    let bad = false;
    if(!u){ markErr($('#lgUser'),'نام کاربری یا شماره موبایل را وارد کنید.'); bad = true; }
    if(!p){ markErr($('#lgPass'),'رمز عبور را وارد کنید.'); bad = true; }
    else if(p.length < 4){ markErr($('#lgPass'),'رمز عبور باید حداقل ۴ کاراکتر باشد.'); bad = true; }
    if(bad){
    ...
    btn.disabled = true; txt.textContent = 'در حال پردازش…';
    setTimeout(()=>{
      const user = DB.users.find(x => x.username === u && x.status === 'active');
      if(!user || p !== '1234'){
        btn.disabled = false; txt.textContent = 'ورود به سامانه';
        alertBox.innerHTML = '<div class="alert a-err shake"><span class="al-ic">'+icon('warn',17)+'</span><div><b>ورود ناموفق.</b> نام کاربری یا رمز عبور اشتباه است. (راهنما: حساب‌های نمایشی پایین صفحه، رمز ۱۲۳۴)</div></div>';
        return;
      }
      ...
      setTimeout(()=>{ location.hash = '#/app/dashboard'; }, 650);
    }, 850);
  });
}
function logout(){
...
async function srvFetch(method, path, body){
  ...
  let r;
  try {
    r = await fetch(SRV.base.replace(/\/+$/,'') + path, { method, headers, body: body !== undefined ? JSON.stringify(body) : undefined });
  } catch(e){
