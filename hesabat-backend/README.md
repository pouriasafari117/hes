...
### ریست کامل دیتابیس (شروع دوباره از صفر)

اگر با حذف دستی رکوردها شمارندهٔ آیدی‌ها ری‌است نشود (مثلاً عضو جدید دوباره از id ~۸ شروع شود)،
باید با `TRUNCATE … RESTART IDENTITY` کل دیتابیس را خالی و شمارنده‌ها را به ۱ برگردانی:

```bash
npm run db:reset       # پاک‌سازی همهٔ جداول + ری‌است شمارنده‌ها (اسکیما دست نمی‌خورد)
```

نکته‌ها:
- این دستور همهٔ مؤسسه‌ها، کاربران، اعضا، وام‌ها و تراکنش‌ها را حذف می‌کند؛ بعد از آن از «ویزارد افتتاح حساب» دوباره مؤسسه بساز.
- مسیر اتصال از `ADMIN_URL` یا `DATABASE_URL` خوانده می‌شود (همان مقداری که برای migrate استفاده می‌کنی).
- اسکیما/فرمت‌بندی جداول تغییری نمی‌کند؛ فقط داده‌ها پاک و آیدی‌ها دوباره از ۱ شمارش داده می‌شوند.
- ریست «فقط دادهٔ دمو (لوکال)» از داخل پنل هم ممکن است: تنظیمات ▸ تب «داده‌ها» ▸ «شروع از صفر».
